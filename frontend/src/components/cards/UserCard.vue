  ...
  <div class="user_card py-3 w-full d-flex align-items-center justify-content-between">
    <div class="flex-center">
      <div class="user_profile d-flex mr-3">
        <img v-if="user" class="rounded-circle w-full h-full" :src="filters.serverLinkFormat(user.image)" alt="">
        <SkeletonLoader v-else width="55px" height="55px" />
      </div>
      <div class="d-flex flex-column">
        <span v-if="user" class="text-color fs-small fw-bold">{{ filters.truncate(user.name, 30) }}</span>
